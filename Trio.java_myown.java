...
*/
	...
	private T findTheSmallest(){
		T one1 = this.item1;
		T one2 = this.item2;
		T one3 = this.item3;	
		T smallest;
		if (one1.compareTo(one2) <= 0  && one1.compareTo(one3) <= 0) {
		    smallest = one1;
		} else if (one2.compareTo(one3) <= 0 && one2.compareTo(one1) <= 0) {
		    smallest = one2;
		} else {
		    smallest = one3;
		}
	...
	public int compareTo(Trio<T> another){
		...
		return this.findTheSmallest().compareTo(another.findTheSmallest());
